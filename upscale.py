...
def check_model_path(model_path):
    if os.path.exists(model_path):
        ...
        print('Error: Model [{:s}] does not exist.'.format(model_path))
        sys.exit(1)
...
class Upscale():
    ...
    def __init__(self, model, input, output, reverse, skip_existing,
                 seamless, cpu, device_id, cache_max_split_depth, binary_alpha,
                  ternary_alpha, alpha_threshold, alpha_boundary_offset, alpha_mode) :
    ...
    def run(self):
        ...
        if not os.path.exists(self.input):
            print('Error: Folder [{:s}] does not exist.'.format(self.input))
            sys.exit(1)
        elif os.path.isfile(self.input):
            print('Error: Folder [{:s}] is a file.'.format(self.input))
            sys.exit(1)
        elif os.path.isfile(self.output):
            print('Error: Folder [{:s}] is a file.'.format(self.output))
            sys.exit(1)
        ...
        for idx, path in enumerate(images, 1):
            ...
            for i, model_path in enumerate(model_chain):
                ...
                if self.cache_max_split_depth and len(split_depths.keys()) > 0:
                    rlt, depth = ops.auto_split_upscale(img, self.upscale, self.last_scale, max_depth=split_depths[i])
                else:
    ...
    def load_model(self,model_path):
        if model_path != self.last_model:
            ...
            if self.in_nc != self.last_in_nc or self.out_nc != self.last_out_nc or nf != self.last_nf or nb != self.last_nb or upscale != self.last_scale or kind != self.last_kind:
                if kind == 'ESRGAN':
                    self.model = arch.RRDB_Net(self.in_nc, self.out_nc, nf, nb, gc=32, upscale=upscale, norm_type=None, act_type='leakyrelu',
                                        mode='CNA', res_scale=1, upsample_mode='upconv')
                elif kind == 'SPSR':
                    self.model = arch.SPSRNet(self.in_nc, self.out_nc, nf, nb, gc=32, upscale=upscale, norm_type=None, act_type='leakyrelu',
...
upscale = Upscale(model=args.model, input=args.input, output=args.output, reverse=args.reverse, skip_existing=args.skip_existing,
                      seamless=args.seamless, cpu=args.cpu, device_id=args.device_id, cache_max_split_depth=args.cache_max_split_depth,
                      binary_alpha=args.binary_alpha, ternary_alpha=args.ternary_alpha, alpha_threshold=args.alpha_threshold,
                      alpha_boundary_offset=args.alpha_boundary_offset, alpha_mode=args.alpha_mode)
upscale.run()
